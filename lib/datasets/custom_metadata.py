...
class MetaDatasetCustom(data.Dataset):
    def __init__(self, data_path, csv_file, img_size, image_set='train', shots=10, phase=1, shuffle=False):

        self.shuffle = shuffle
        self.img_size = img_size
        self.phase = phase
        self.shot_path = os.path.join(data_path, 'shots.csv')
        self.shots = shots
        if phase == 2:
            self.shots = shots * 3
        ...
        if phase == 1:
            self.metaclass = [c for c in np.unique(df.cls).tolist() if c not in cfg.CUSTOM_NOVEL_CLASSES]
        else:
        ...
        # Used for reproduce results with the same few-shot samples
    ...
    def get_prndata(self):
        ...
        for index in self._image_index:
            ...
            for ix in range(num_objs):
                ...
                cls = objs.iloc[ix]['cls']

                if objs.iloc[ix]['difficult']:
                    continue
